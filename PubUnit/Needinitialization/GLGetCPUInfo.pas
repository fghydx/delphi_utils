...
type
  TGLCPUIdFormat = (ifContinuous, ifDashed);
  {* CPU序列号与信息串显示样式
   |<PRE>
     ifContinuous:  -连续型
     ifDashed:      -使用分割符'-'分割
  ...
  private
    ...
    procedure SetCPUIdFormat(ACPUIdFormat: TGLCPUIdFormat);
  ...
  public
    ...
    property CPUIdFormat: TGLCPUIdFormat read FCPUIdFormat write SetCPUIdFormat;
    {* CPU 序列号显示样式}
...
var
  FdtStartByProcess,FdtCPUPrevByProcess:Int64;
...
var
  ...
  CurrProc: THandle;
  SysInfo: TSystemInfo;
  ProcessAffinityOld: NativeUInt;
  ProcessAffinity: NativeUInt;
  SystemAffinity: NativeUInt;
...
procedure TGLCpuId.SetCPUIdFormat(ACPUIdFormat: TGLCPUIdFormat);
begin
...
// 获得单 CPU 系统 ID
function TGLCpuId.GetFirstCPUId: string;
begin
...
// 得到第几个 CPU 的序列号
function TGLCpuId.GetCPUId(Index: Integer): string;
begin
  Result := '';
  // 保证 FCPUIds 索引的合法性
...
function TGLCpuId.GetAverageCPUUsage: Integer;
begin
...
function TGLCpuId.GetCPUUsage(Index: Integer): Integer;
begin
...
function TGLCpuId.GetFirstCPUUsage: Integer;
begin
...
function TGLCpuId.RefreshCPUUsages: Cardinal;
var
  CpuCnt: Cardinal;
  I: integer;
  Spt: Pointer;
...
procedure TGLCpuId.CpuUsageTimer(Sender: TObject);
var
  I: Integer;
begin
  RefreshCPUUsages;
...
function TGLCpuId.GetCPUOem(Index: Integer): string;
begin
  Result := '';
  // 保证 FCPUIds 索引的合法性
...
function TGLCpuId.GetFirstCPUOem: string;
begin
  if FCPUOems.Count > 0 then
    Result := FCPUOems.Strings[0];
end;

function TGLCpuId.GetSupportCPUId(Index: Integer): Boolean;
begin
  Result := False;
...
function TGLCpuId.GetSupportCPUSn(Index: Integer): Boolean;
begin
  Result := False;
  // 保证 FSupportCpuIds 索引的合法性
...
function TGLCpuId.GetFirstSupportCPUId: Boolean;
begin
  Result := False;
...
function TGLCpuId.GetFirstSupportCPUSn: Boolean;
begin
  Result := False;
...
function TGLCpuId.GetCPUInfoString(Index: Integer): string;
begin
  Result := '';
...
function TGLCpuId.GetFirstCPUInfoString: string;
begin
