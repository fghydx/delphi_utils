...
const
  XMLanalyTypeStr:array[0..1] of string = ('MSXML','Open XML');
...
{ TXMLNode }
  ...
  IXMLNodeAccess = interface(IXMLNode)
    ...
    function GetNodes(Index: Integer): IXMLNodeAccess;
    ...
    property Nodes[Index: Integer]: IXMLNodeAccess read GetNodes;
  end;
...
function TXMLNode.GetNodes(Index: Integer): IXMLNodeAccess;
begin
  if FDocument.DOMVendor.Description=XMLanalyTypeStr[0] then
  begin
    if Index<ChildNodes.Count then
      Result := ChildNodes.Get(Index) as IXMLNodeAccess;
  end else
  begin
    if Index*2+1 < ChildNodes.Count then
      Result := ChildNodes.Get(Index*2+1) as IXMLNodeAccess;
  end;
end;
