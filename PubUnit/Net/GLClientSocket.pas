...
uses SysUtils, Windows, Messages, Classes, GLWinSockX, SyncObjs;
...
type
  ...
  private
    ...
    FClient: Boolean;
    FQueueSize: Integer;
    FTimeOut,FConnectTimeOut: Integer;
...
begin
  ...
  Event(Self, seListen);
  CheckSocketResult(GLWinsockX.listen(FSocket, QueueSize), 'listen');
  FLookupState := lsIdle;
  FConnected := True;
end;

procedure TCustomWinSocket.DoOpen;
  function ConnectWithTimeout:Integer;
  var
    TimeVal:TTimeVal;
    Mode:Cardinal;
    FDSet: TFDSet;
  ...
  begin
    Result := 0;
    Mode := 1;
    result := ioctlsocket(FSocket,FIONBIO,Mode);
    if (result <> NO_ERROR) then Exit;

    Result := GLWinsockX.connect(FSocket, @FAddr, SizeOf(FAddr));
    if (result = SOCKET_ERROR) then
    begin
      TimeVal.tv_sec := FConnectTimeOut div 1000;
      TimeVal.tv_usec := (FConnectTimeOut mod 1000) * 1000;
      ...
      begin
        optLen := 0;
        err := 0;
        getsockopt(FSocket, SOL_SOCKET, SO_ERROR, @err, optLen);
        Result := err;
        if Result<>0 then
        begin
          SetLastError(Result);
...
begin
  ...
  if FConnectTimeOut<=0 then
    CheckSocketResult(GLWinsockX.connect(FSocket, @FAddr, SizeOf(FAddr)), 'connect')
  else
  begin
    CheckSocketResult(ConnectWithTimeout,'Timeout when connection');
  end;
  FLookupState := lsIdle;
...
//如果是阻塞类型，设置超时时长
      setsockopt(FSocket,SOL_SOCKET,SO_RCVTIMEO,@FTimeOut,SizeOf(FTimeOut));
      setsockopt(FSocket,SOL_SOCKET,SO_SNDTIMEO,@FTimeOut,SizeOf(FTimeOut));
      DoOpen;
    end else
...
begin
  ...
  FClientSocket.ConnectTimeOut := Value;
end;

procedure TClientSocket.SetTimeOut(const Value: Integer);
begin
  FTimeOut := Value;
