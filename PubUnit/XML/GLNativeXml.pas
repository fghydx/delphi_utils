...
type
  ...
  TXmlNodeEvent = procedure(Sender: TObject; Node: TGLXmlNode) of object;
  ...
  TXmlNodeCompareEvent = function(Sender: TObject; Node1, Node2: TGLXmlNode; Info: TPointer): integer of object;

  // Pass a function of this kind to TXmlNode.SortChildNodes. The function should
  // return -1 if Node1 < Node2, 0 if Node1 = Node2 and 1 if Node1 > Node2.
  TXMLNodeCompareFunction = function(Node1, Node2: TGLXmlNode; Info: TPointer): integer;
  ...
  private
    ...
    FParent: TGLXmlNode;              // Pointer to parent element
    FDocument: TGLNativeXml;          // Pointer to parent XmlDocument
  ...
  protected
    ...
    function GetNodes(Index: integer): TGLXmlNode; virtual;
  ...
  public
    ...
    constructor CreateNameValue(ADocument: TGLNativeXml; const AName, AValue: UTF8String); virtual;
    // \Create a new TXmlNode with XML element type AType. ADocument must be the
    // TNativeXml that is going to hold this new node.
    constructor CreateType(ADocument: TGLNativeXml; AType: TXmlElementType); virtual;
    ...
    function FindNode(const NodeName: UTF8String): TGLXmlNode;
    ...
    function IsEqualTo(ANode: TGLXmlNode; Options: TXmlCompareOptions; MismatchNodes: TList = nil): boolean;
    ...
    function NodeAdd(ANode: TGLXmlNode): integer; virtual;
    ...
    function NodeByName(const AName: UTF8String): TGLXmlNode; virtual;
    ...
    function NodeExtract(ANode: TGLXmlNode): TGLXmlNode; virtual;
    ...
    function NodeFindOrCreate(const AName: UTF8String): TGLXmlNode; virtual;
    ...
    function NodeIndexOf(ANode: TGLXmlNode): integer;
    // Insert the node ANode at location Index in the list.
    procedure NodeInsert(Index: integer; ANode: TGLXmlNode); virtual;
    // \Create a new node with AName, add it to the subnode list, and return a
    // pointer to it.
    function NodeNew(const AName: UTF8String): TGLXmlNode; virtual;
    // \Create a new node with AName, and insert it into the subnode list at location
    // Index, and return a pointer to it.
    function NodeNewAtIndex(Index: integer; const AName: UTF8String): TGLXmlNode; virtual;
    ...
    function NodeRemove(ANode: TGLXmlNode): integer;
    ...
    property Document: TGLNativeXml read FDocument write FDocument;
    ...
    property Parent: TGLXmlNode read FParent write FParent;
    ...
    property Nodes[Index: integer]: TGLXmlNode read GetNodes; default;
  ...
  private
    function GetItems(Index: Integer): TGLXmlNode;
    procedure SetItems(Index: Integer; const Value: TGLXmlNode);
  public
    // Return the first node in the list that has an attribute with AName, AValue
    function ByAttribute(const AName, AValue: UTF8String): TGLXmlNode;
    property Items[Index: Integer]: TGLXmlNode read GetItems write SetItems; default;
  end;
  ...
  // is produced.
  TGLNativeXml = class(TPersistent)
  private
    ...
    FRootNodes: TGLXmlNode;           // Root nodes in the document (which contains one normal element that is the root)
    ...
    procedure DoNodeNew(Node: TGLXmlNode);
    procedure DoNodeLoaded(Node: TGLXmlNode);
    ...
    function GetStyleSheetNode: TGLXmlNode;
    function GetUtf8Encoded: boolean;
  protected
    procedure CopyFrom(Source: TGLNativeXml); virtual;
    procedure DoProgress(Size: integer);
    function LineFeed: UTF8String; virtual;
    procedure ParseDTD(ANode: TGLXmlNode; S: TStream); virtual;
    procedure ReadFromStream(S: TStream); virtual;
    procedure WriteToStream(S: TStream); virtual;
  ...
  public
    ...
    property Root: TGLXmlNode read GetRoot;
    ...
    property RootNodeList: TGLXmlNode read FRootNodes;
    ...
    property StyleSheetNode: TGLXmlNode read GetStyleSheetNode;
...
function TGLXmlNode.AbortParsing: boolean;
begin
  Result := assigned(Document) and Document.AbortParsing;
end;

procedure TGLXmlNode.Assign(Source: TPersistent);
var
  i: integer;
  Node: TGLXmlNode;
begin
  if Source is TGLXmlNode then
  begin
    // Clear first
    Clear;

    // Properties
    FElementType := TGLXmlNode(Source).FElementType;
    FName := TGLXmlNode(Source).FName;
    FTag := TGLXmlNode(Source).FTag;
    FValue := TGLXmlNode(Source).FValue;

    // Attributes
    if assigned(TGLXmlNode(Source).FAttributes) then
    begin
      CheckCreateAttributesList;
      FAttributes.Assign(TGLXmlNode(Source).FAttributes);
    end;

    // Nodes
    for i := 0 to TGLXmlNode(Source).NodeCount - 1 do
    begin
      Node := NodeNew('');
      Node.Assign(TGLXmlNode(Source).Nodes[i]);
    end;
  end else
    if Source is TGLNativeXml then
    begin
      Assign(TGLNativeXml(Source).FRootNodes);
    end else
      inherited;
end;

procedure TGLXmlNode.AttributeAdd(const AName, AValue: UTF8String);
var
  Attr: UTF8String;
begin
...
procedure TGLXmlNode.AttributeAdd(const AName: UTF8String; AValue: integer);
begin
  AttributeAdd(AName, IntToUTF8Str(AValue));
end;

procedure TGLXmlNode.AttributeDelete(Index: integer);
begin
  if (Index >= 0) and (Index < AttributeCount) then
    FAttributes.Delete(Index);
end;

procedure TGLXmlNode.AttributeExchange(Index1, Index2: integer);
var
  Temp: UTF8String;
begin
...
function TGLXmlNode.AttributeIndexByname(const AName: UTF8String): integer;
...
procedure TGLXmlNode.AttributesClear;
begin
  FreeAndNil(FAttributes);
end;

function TGLXmlNode.BufferLength: integer;
var
  BufData: UTF8String;
  BufPos: integer;
begin
...
procedure TGLXmlNode.BufferRead(var Buffer{$IFDEF CLR}: TBytes{$ENDIF}; Count: Integer);
...
procedure TGLXmlNode.BufferWrite(const Buffer{$IFDEF CLR}: TBytes{$ENDIF}; Count: Integer);
...
procedure TGLXmlNode.ChangeDocument(ADocument: TGLNativeXml);
var
  i: integer;
begin
  FDocument := ADocument;
...
procedure TGLXmlNode.CheckCreateAttributesList;
begin
  if not assigned(FAttributes) then
    FAttributes := TsdUTF8StringList.Create;
end;

procedure TGLXmlNode.Clear;
begin
  // Name + value
  FName  := '';
  FValue := '';
...
function TGLXmlNode.CompareNodeName(const NodeName: UTF8String): integer;
begin
...
constructor TGLXmlNode.Create(ADocument: TGLNativeXml);
begin
  inherited Create;
  FDocument := ADocument;
end;

constructor TGLXmlNode.CreateName(ADocument: TGLNativeXml; const AName: UTF8String);
begin
  Create(ADocument);
  Name := AName;
end;

constructor TGLXmlNode.CreateNameValue(ADocument: TGLNativeXml; const AName, AValue: UTF8String);
begin
  Create(ADocument);
  Name := AName;
...
constructor TGLXmlNode.CreateType(ADocument: TGLNativeXml;
...
procedure TGLXmlNode.Delete;
begin
  if assigned(Parent) then
    Parent.NodeRemove(Self);
end;

procedure TGLXmlNode.DeleteEmptyAttributes;
var
  i: integer;
  V: UTF8String;
begin
...
procedure TGLXmlNode.DeleteEmptyNodes;
var
  i: integer;
  Node: TGLXmlNode;
begin
...
function TGLXmlNode.FindNode(const NodeName: UTF8String): TGLXmlNode;
// Find the first node which has name NodeName. Contrary to the NodeByName
// function, this function will search the whole subnode tree, using the
// DepthFirst method.
var
...
procedure TGLXmlNode.FindNodes(const NodeName: UTF8String; const AList: TList);
  // local
  procedure FindNodesRecursive(ANode: TGLXmlNode; AList: TList);
  var
    i: integer;
  begin
    with ANode do
...
function TGLXmlNode.FloatAllowScientific: boolean;
begin
...
function TGLXmlNode.FloatSignificantDigits: integer;
begin
  if assigned(Document) then
...
function TGLXmlNode.FromAnsiString(const s: AnsiString): UTF8String;
begin
  Result := sdAnsiToUtf8Old(s)
end;

function TGLXmlNode.FromUnicodeString(const W: UnicodeString): UTF8String;
begin
  Result := sdUnicodeToUtf8(W)
end;

function TGLXmlNode.GetAttributeByName(const AName: UTF8String): UTF8String;
begin
  if assigned(FAttributes) then
...
function TGLXmlNode.GetAttributeByNameWide(const AName: UTF8String): UnicodeString;
begin
  Result := ToUnicodeString(GetAttributeByName(AName));
end;

function TGLXmlNode.GetAttributeCount: integer;
begin
  if assigned(FAttributes) then
...
function TGLXmlNode.GetAttributeName(Index: integer): UTF8String;
begin
  if (Index >= 0) and (Index < AttributeCount) then
    Result := FAttributes.Names[Index];
end;

function TGLXmlNode.GetAttributePair(Index: integer): UTF8String;
begin
  if (Index >= 0) and (Index < AttributeCount) then
    Result := FAttributes[Index];
end;

function TGLXmlNode.GetAttributeValue(Index: integer): UTF8String;
var
  P: integer;
  S: UTF8String;
begin
...
function TGLXmlNode.GetAttributeValueAsInteger(Index: integer): integer;
begin
  Result := StrToIntDef(string(GetAttributeValue(Index)), 0);
end;

function TGLXmlNode.GetAttributeValueAsUnicodeString(Index: integer): UnicodeString;
begin
  Result := ToUnicodeString(GetAttributeValue(Index));
end;

function TGLXmlNode.GetAttributeValueDirect(Index: integer): UTF8String;
var
  P: integer;
  S: UTF8String;
begin
...
function TGLXmlNode.GetBinaryEncoding: TBinaryEncodingType;
begin
  Result := xbeBinHex;
  if assigned(Document) then
...
function TGLXmlNode.GetBinaryString: RawByteString;
...
function TGLXmlNode.GetCascadedName: UTF8String;
...
function TGLXmlNode.GetFullPath: UTF8String;
...
function TGLXmlNode.GetIndent: UTF8String;
var
  i: integer;
begin
  if assigned(Document) then
...
function TGLXmlNode.GetLineFeed: UTF8String;
begin
  if assigned(Document) then
  begin
    case Document.XmlFormat of
...
function TGLXmlNode.GetNodeCount: integer;
begin
...
function TGLXmlNode.GetNodes(Index: integer): TGLXmlNode;
begin
  if (Index >= 0) and (Index < NodeCount) then
    Result := TGLXmlNode(FNodes[Index])
  else
    Result := nil;
end;

function TGLXmlNode.GetTotalNodeCount: integer;
var
  i: integer;
begin
  Result := NodeCount;
...
function TGLXmlNode.GetTreeDepth: integer;
begin
  Result := -1;
  if assigned(Parent) then
...
function TGLXmlNode.GetValueAsBool: boolean;
begin
  Result := sdStringToBool(FValue);
end;

function TGLXmlNode.GetValueAsDateTime: TDateTime;
begin
  Result := sdDateTimeFromString(ValueAsString, UseLocalBias);
end;

function TGLXmlNode.GetValueAsFloat: double;
var
  Code: integer;
begin
...
function TGLXmlNode.GetValueAsInt64: int64;
begin
  Result := StrToInt64(string(FValue));
end;

function TGLXmlNode.GetValueAsInteger: integer;
begin
  Result := StrToInt(string(FValue));
end;

function TGLXmlNode.GetValueAsString: UTF8String;
begin
...
function TGLXmlNode.GetValueAsUnicodeString: UnicodeString;
begin
  Result := ToUnicodeString(ValueAsString);
end;

function TGLXmlNode.GetWriteOnDefault: boolean;
begin
  Result := True;
  if assigned(Document) then
...
function TGLXmlNode.HasAttribute(const AName: UTF8String): boolean;
begin
  if assigned(FAttributes) then
...
function TGLXmlNode.IndexInParent: integer;
...
function TGLXmlNode.IsClear: boolean;
begin
  Result := (Length(FName) = 0) and IsEmpty;
end;

function TGLXmlNode.IsEmpty: boolean;
begin
  Result := (Length(FValue) = 0) and (NodeCount = 0) and (AttributeCount = 0);
end;

function TGLXmlNode.IsEqualTo(ANode: TGLXmlNode; Options: TXmlCompareOptions;
...
function TGLXmlNode.NodeAdd(ANode: TGLXmlNode): integer;
begin
...
function TGLXmlNode.NodeByAttributeValue(const NodeName, AttribName, AttribValue: UTF8String;
  ShouldRecurse: boolean): TGLXmlNode;
// This function returns a pointer to the first subnode that has an attribute with
...
function TGLXmlNode.NodeByElementType(ElementType: TXmlElementType): TGLXmlNode;
var
  i: integer;
begin
  Result := nil;
...
function TGLXmlNode.NodeByName(const AName: UTF8String): TGLXmlNode;
var
  i: integer;
begin
  Result := nil;
...
procedure TGLXmlNode.NodeDelete(Index: integer);
begin
  if (Index >= 0) and (Index < NodeCount) then
  begin
    TGLXmlNode(FNodes[Index]).Free;
    FNodes.Delete(Index);
  end;
end;

procedure TGLXmlNode.NodeExchange(Index1, Index2: integer);
begin
...
function TGLXmlNode.NodeExtract(ANode: TGLXmlNode): TGLXmlNode;
var
  Index: integer;
begin
...
function TGLXmlNode.NodeFindOrCreate(const AName: UTF8String): TGLXmlNode;
...
function TGLXmlNode.NodeIndexByName(const AName: UTF8String): integer;
begin
  Result := 0;
...
function TGLXmlNode.NodeIndexByNameFrom(const AName: UTF8String; AFrom: integer): integer;
begin
  Result := AFrom;
  while Result < NodeCount do
  begin
...
function TGLXmlNode.NodeIndexOf(ANode: TGLXmlNode): integer;
begin
...
procedure TGLXmlNode.NodeInsert(Index: integer; ANode: TGLXmlNode);
// Insert the node ANode at location Index in the list.
begin
  if not assigned(ANode) then
    exit;
...
function TGLXmlNode.NodeNew(const AName: UTF8String): TGLXmlNode;
// Add a new child node and return its pointer
begin
  Result := Nodes[NodeAdd(TGLXmlNode.CreateName(Document, AName))];
end;

function TGLXmlNode.NodeNewAtIndex(Index: integer; const AName: UTF8String): TGLXmlNode;
// Create a new node with AName, and insert it into the subnode list at location
// Index, and return a pointer to it.
begin
  if (Index >= 0) and (Index <= NodeCount) then
  begin
    Result := TGLXmlNode.CreateName(Document, AName);
    NodeInsert(Index, Result);
  end else
    Result := nil;
end;

function TGLXmlNode.NodeRemove(ANode: TGLXmlNode): integer;
begin
...
procedure TGLXmlNode.NodesByName(const AName: UTF8String; const AList: TList);
...
procedure TGLXmlNode.NodesClear;
var
  i: integer;
begin
  for i := 0 to NodeCount - 1 do
    TGLXmlNode(FNodes[i]).Free;
  FreeAndNil(FNodes);
end;

procedure TGLXmlNode.ParseTag(const AValue: UTF8String; TagStart, TagClose: integer);
var
...
function TGLXmlNode.QualifyAsDirectNode: boolean;
...
function TGLXmlNode.ReadAttributeBool(const AName: UTF8String; ADefault: boolean): boolean;
var
  V: UTF8String;
begin
  Result := ADefault;
...
function TGLXmlNode.ReadAttributeDateTime(const AName: UTF8String; ADefault: TDateTime): TDateTime;
var
  V: UTF8String;
begin
  Result := ADefault;
...
function TGLXmlNode.ReadAttributeFloat(const AName: UTF8String; ADefault: double): double;
var
  V: UTF8String;
  Code: integer;
begin
...
function TGLXmlNode.ReadAttributeInteger(const AName: UTF8String; ADefault: integer): integer;
begin
  Result := StrToIntDef(string(AttributeByName[AName]), ADefault);
end;

function TGLXmlNode.ReadAttributeInt64(const AName: UTF8String; ADefault: int64): int64;
begin
  Result := StrToInt64Def(string(AttributeByName[AName]), ADefault);
end;

function TGLXmlNode.ReadAttributeString(const AName: UTF8String; const ADefault: UTF8String): UTF8String;
begin
...
function TGLXmlNode.ReadBool(const AName: UTF8String; ADefault: boolean): boolean;
var
  Index: integer;
begin
  Result := ADefault;
...
function TGLXmlNode.ReadDateTime(const AName: UTF8String; ADefault: TDateTime): TDateTime;
...
function TGLXmlNode.ReadFloat(const AName: UTF8String; ADefault: double): double;
var
  Index: integer;
begin
  Result := ADefault;
...
procedure TGLXmlNode.ReadFromStream(S: TStream);
...
var
  ...
  begin
    ...
    begin
      ...
      if L > 0 then
      begin
        Node := TGLXmlNode.CreateType(Document, xeoCharData);
        Node.ValueDirect := V;
        NodeAdd(Node);
      end;
      ValuePos := 0;
              ...
              repeat
                ...
                begin
                  ...
                  begin
                    ...
                    HasSubTags := True;
                    S.Seek(-2, soCurrent);
                    Node := TGLXmlNode.Create(Document);
                    NodeAdd(Node);
...
procedure TGLXmlNode.ReadFromString(const AValue: UTF8String);
var
  S: TStream;
begin
...
function TGLXmlNode.ReadInt64(const AName: UTF8String; ADefault: int64): int64;
var
  Index: integer;
begin
  Result := ADefault;
...
function TGLXmlNode.ReadInteger(const AName: UTF8String; ADefault: integer): integer;
var
  Index: integer;
begin
  Result := ADefault;
...
function TGLXmlNode.ReadString(const AName: UTF8String; const ADefault: UTF8String): UTF8String;
var
  Index: integer;
begin
  Result := ADefault;
...
function TGLXmlNode.ReadUnicodeString(const AName: UTF8String; const ADefault: UnicodeString): UnicodeString;
begin
  Result := ToUnicodeString(ReadString(AName, FromUnicodeString(ADefault)));
end;

procedure TGLXmlNode.ResolveEntityReferences;
// Replace any entity references by the entities, and parse the new content if any
  ...
  // local
  function ReplaceEntityReferenceByNodes(ARoot: TGLXmlNode; const AValue: UTF8String; var InsertPos: integer; var Text1, Text2: UTF8String): boolean;
  var
...
procedure TGLXmlNode.SetAttributeByName(const AName, Value: UTF8String);
begin
  CheckCreateAttributesList;
  FAttributes.Values[AName] := sdUTF8QuotedString(sdUTF8EscapeString(Value));
end;

procedure TGLXmlNode.SetAttributeByNameWide(const AName: UTF8String; const Value: UnicodeString);
begin
  SetAttributeByName(AName, FromUnicodeString(Value));
end;

procedure TGLXmlNode.SetAttributeName(Index: integer; const Value: UTF8String);
var
  S: UTF8String;
  P: integer;
begin
...
procedure TGLXmlNode.SetAttributeValue(Index: integer; const Value: UTF8String);
begin
  if (Index >= 0) and (Index < AttributeCount) then
...
procedure TGLXmlNode.SetAttributeValueAsInteger(Index: integer; const Value: integer);
begin
  SetAttributeValue(Index, IntToUTF8Str(Value));
end;

procedure TGLXmlNode.SetAttributeValueAsUnicodeString(Index: integer;
  const Value: UnicodeString);
begin
  SetAttributeValue(Index, FromUnicodeString(Value));
end;

procedure TGLXmlNode.SetAttributeValueDirect(Index: integer; const Value: UTF8String);
begin
  if (Index >= 0) and (Index < AttributeCount) then
    FAttributes[Index] := AttributeName[Index] + '=' +
...
procedure TGLXmlNode.SetBinaryEncoding(const Value: TBinaryEncodingType);
begin
  if assigned(Document) then
    Document.BinaryEncoding := Value;
end;

procedure TGLXmlNode.SetBinaryString(const Value: RawByteString);
var
  OldEncoding: TBinaryEncodingType;
begin
  // Set to base64
...
procedure TGLXmlNode.SetName(const Value: UTF8String);
var
  i: integer;
begin
  if FName <> Value then
...
procedure TGLXmlNode.SetValueAsBool(const Value: boolean);
begin
  FValue := sdStringFromBool(Value);
end;

procedure TGLXmlNode.SetValueAsDateTime(const Value: TDateTime);
begin
  ValueAsString := sdDateTimeToString(Value, UseLocalBias);
end;

procedure TGLXmlNode.SetValueAsFloat(const Value: double);
begin
  FValue := sdWriteNumber(Value, FloatSignificantDigits, FloatAllowScientific);
end;

procedure TGLXmlNode.SetValueAsInt64(const Value: int64);
begin
  FValue := Int64ToUTF8Str(Value);
end;

procedure TGLXmlNode.SetValueAsInteger(const Value: integer);
begin
  FValue := IntToUTF8Str(Value);
end;

procedure TGLXmlNode.SetValueAsString(const AValue: UTF8String);
begin
  FValue := sdUTF8EscapeString(AValue);
end;

procedure TGLXmlNode.SetValueAsUnicodeString(const Value: UnicodeString);
begin
  ValueAsString := FromUnicodeString(Value);
end;

procedure TGLXmlNode.SortChildNodes(Compare: TXMLNodeCompareFunction; Info: TPointer);
// Sort the child nodes using the quicksort algorithm
  //local
  function DoNodeCompare(Node1, Node2: TGLXmlNode): integer;
  begin
...
function TGLXmlNode.ToUnicodeString(const s: UTF8String): UnicodeString;
begin
  Result := sdUtf8ToUnicode(s)
end;

function TGLXmlNode.UnescapeString(const AValue: UTF8String): UTF8String;
begin
  Result := sdUTF8UnEscapeString(AValue)
end;

function TGLXmlNode.UseFullNodes: boolean;
begin
  Result := False;
  if assigned(Document) then
...
function TGLXmlNode.UseLocalBias: Boolean;
begin
  Result := False;
  if Assigned(Document) then
...
function TGLXmlNode.ValueAsBoolDef(ADefault: boolean): boolean;
var
  Ch: AnsiChar;
begin
  Result := ADefault;
...
function TGLXmlNode.ValueAsDateTimeDef(ADefault: TDateTime): TDateTime;
begin
  Result := sdDateTimeFromStringDefault(ValueAsString, ADefault, UseLocalBias);
end;

function TGLXmlNode.ValueAsFloatDef(ADefault: double): double;
var
  Code: integer;
begin
  try
...
function TGLXmlNode.ValueAsInt64Def(ADefault: int64): int64;
begin
  Result := StrToInt64Def(string(FValue), ADefault);
end;

function TGLXmlNode.ValueAsIntegerDef(ADefault: integer): integer;
begin
  Result := StrToIntDef(string(FValue), ADefault);
end;

procedure TGLXmlNode.WriteAttributeBool(const AName: UTF8String; AValue: boolean; ADefault: boolean);
var
  Index: integer;
begin
...
procedure TGLXmlNode.WriteAttributeDateTime(const AName: UTF8String; AValue, ADefault: TDateTime);
var
  Index: integer;
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TGLXmlNode.WriteAttributeFloat(const AName: UTF8String; AValue, ADefault: double);
var
  Index: integer;
  S: UTF8String;
begin
...
procedure TGLXmlNode.WriteAttributeInteger(const AName: UTF8String; AValue: integer; ADefault: integer);
var
  Index: integer;
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TGLXmlNode.WriteAttributeInt64(const AName: UTF8String; const AValue: int64; ADefault: int64);
var
  Index: integer;
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TGLXmlNode.WriteAttributeString(const AName, AValue, ADefault: UTF8String);
var
  Index: integer;
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TGLXmlNode.WriteBool(const AName: UTF8String; AValue: boolean; ADefault: boolean);
const
  cBoolValues: array[boolean] of UTF8String = ('False', 'True');
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TGLXmlNode.WriteDateTime(const AName: UTF8String; AValue, ADefault: TDateTime);
// Date MUST always be written in this format:
// YYYY-MM-DD (if just date) or
// YYYY-MM-DDThh:mm:ss.sssZ (if date and time. The Z stands for universal time
// zone. Since Delphi's TDateTime does not give us a clue about the timezone,
...
procedure TGLXmlNode.WriteFloat(const AName: UTF8String; AValue: double; ADefault: double);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
...
procedure TGLXmlNode.WriteHex(const AName: UTF8String; AValue, Digits: integer; ADefault: integer);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
...
function TGLXmlNode.WriteInnerTag: UTF8String;
...
procedure TGLXmlNode.WriteInt64(const AName: UTF8String; AValue, ADefault: int64);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
...
procedure TGLXmlNode.WriteInteger(const AName: UTF8String; AValue: integer; ADefault: integer);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
...
procedure TGLXmlNode.WriteString(const AName, AValue: UTF8String; const ADefault: UTF8String);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
      ValueAsString := AValue;
...
procedure TGLXmlNode.WriteToStream(S: TStream);
var
  i: integer;
  Indent: UTF8String;
  LFeed: UTF8String;
  Line: UTF8String;
  ThisNode, NextNode: TGLXmlNode;
  AddLineFeed: boolean;
begin
  Indent := GetIndent;
  LFeed := GetLineFeed;
...
function TGLXmlNode.WriteToString: UTF8String;
var
...
procedure TGLXmlNode.WriteUnicodeString(const AName: UTF8String;
...
function TXmlNodeList.ByAttribute(const AName, AValue: UTF8String): TGLXmlNode;
var
  i: integer;
begin
  for i := 0 to Count - 1 do
...
function TXmlNodeList.GetItems(Index: Integer): TGLXmlNode;
begin
  Result := TGLXmlNode(Get(Index));
end;

procedure TXmlNodeList.SetItems(Index: Integer; const Value: TGLXmlNode);
begin
...
procedure TGLNativeXml.Assign(Source: TPersistent);
  // local
  procedure SetDocumentRecursively(ANode: TGLXmlNode; ADocument: TGLNativeXml);
  var
    i: integer;
  begin
...
begin
  ...
  begin
    ...
    FBinaryEncoding := TGLNativeXml(Source).FBinaryEncoding;
    FDropCommentsOnParse := TGLNativeXml(Source).FDropCommentsOnParse;
    FExternalEncoding := TGLNativeXml(Source).FExternalEncoding;
    FParserWarnings := TGLNativeXml(Source).FParserWarnings;
    FIndentString := TGLNativeXml(Source).FIndentString;
    FUseFullNodes := TGLNativeXml(Source).FUseFullNodes;
    FUseLocalBias := TGLNativeXml(Source).FUseLocalBias;
    FWriteOnDefault := TGLNativeXml(Source).FWriteOnDefault;
    FXmlFormat := TGLNativeXml(Source).FXmlFormat;
    // Assign root
    FRootNodes.Assign(TGLNativeXml(Source).FRootNodes);
...
begin
  ...
  Node := TGLXmlNode.CreateType(Self, xeoDeclaration);
  Node.Name := 'xml';
...
procedure TGLNativeXml.CopyFrom(Source: TGLNativeXml);
begin
...
constructor TGLNativeXml.Create;
begin
  inherited Create;
  FRootNodes := TGLXmlNode.Create(Self);
  Clear;
end;

constructor TGLNativeXml.CreateName(const ARootName: UTF8String);
begin
  Create;
  Root.Name := ARootName;
end;

destructor TGLNativeXml.Destroy;
begin
  FreeAndNil(FRootNodes);
  inherited;
end;

procedure TGLNativeXml.DoNodeLoaded(Node: TGLXmlNode);
begin
  if assigned(FOnNodeLoaded) then
    FOnNodeLoaded(Self, Node);
end;

procedure TGLNativeXml.DoNodeNew(Node: TGLXmlNode);
begin
  if assigned(FOnNodeNew) then
    FOnNodeNew(Self, Node);
end;

procedure TGLNativeXml.DoProgress(Size: integer);
begin
  if assigned(FOnProgress) then
    FOnProgress(Self, Size);
end;

procedure TGLNativeXml.DoUnicodeLoss(Sender: TObject);
begin
  if assigned(FOnUnicodeLoss) then
    FOnUnicodeLoss(Self);
end;

function TGLNativeXml.GetCommentString: UTF8String;
...
function TGLNativeXml.GetEncodingString: UTF8String;
begin
  Result := '';
...
function TGLNativeXml.GetEntityByName(AName: UTF8String): UTF8String;
var
  i, j: integer;
begin
  Result := '';
...
function TGLNativeXml.GetRoot: TGLXmlNode;
begin
  Result := FRootNodes.NodeByElementType(xeoNormal);
end;

function TGLNativeXml.GetStyleSheetNode: TGLXmlNode;
begin
  Result := FRootNodes.NodeByElementType(xeoStylesheet);
  if not assigned(Result) then
  begin
    // Add a stylesheet node as second one if none present
    Result := TGLXmlNode.CreateType(Self, xeoStyleSheet);
    FRootNodes.NodeInsert(1, Result);
  end;
end;

function TGLNativeXml.GetUtf8Encoded: boolean;
begin
  Result := True;
end;

function TGLNativeXml.GetVersionString: UTF8String;
begin
  Result := '';
  if FRootNodes.NodeCount > 0 then
    if FRootNodes[0].ElementType = xeoDeclaration then
...
function TGLNativeXml.IsEmpty: boolean;
var
  R: TGLXmlNode;
begin
  Result := True;
...
function TGLNativeXml.LineFeed: UTF8String;
begin
  case XmlFormat of
  xfoReadable:
    Result := #13#10;
...
procedure TGLNativeXml.LoadFromFile(const AFileName: string);
var
  S: TStream;
begin
...
procedure TGLNativeXml.LoadFromStream(Stream: TStream);
var
...
procedure TGLNativeXml.ParseDTD(ANode: TGLXmlNode; S: TStream);
...
procedure TGLNativeXml.ReadFromStream(S: TStream);
var
  i: integer;
  Node: TGLXmlNode;
  Enc: UTF8String;
...
procedure TGLNativeXml.ReadFromString(const AValue: UTF8String);
var
  S: TStream;
begin
  S := TsdUTF8StringStream.Create(AValue);
...
procedure TGLNativeXml.ResolveEntityReferences;
begin
  if assigned(Root) then
    Root.ResolveEntityReferences;
end;

procedure TGLNativeXml.SaveToFile(const AFileName: string);
var
  S: TStream;
begin
...
procedure TGLNativeXml.SaveToStream(Stream: TStream);
var
...
procedure TGLNativeXml.SetCommentString(const Value: UTF8String);
...
begin
  ...
  begin
    Node := TGLXmlNode.CreateType(Self, xeoComment);
...
procedure TGLNativeXml.SetDefaults;
begin
  // Defaults
...
procedure TGLNativeXml.SetEncodingString(const Value: UTF8String);
var
  Node: TGLXmlNode;
begin
...
procedure TGLNativeXml.SetVersionString(const Value: UTF8String);
var
  Node: TGLXmlNode;
begin
...
procedure TGLNativeXml.WriteToStream(S: TStream);
var
  i: integer;
begin
...
function TGLNativeXml.WriteToString: UTF8String;
var
  S: TsdUTF8StringStream;
begin
  S := TsdUTF8StringStream.Create('');
